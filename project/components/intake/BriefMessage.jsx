...
export function BriefMessage({ role = 'ai', index, children, pending, style }) {
  const ai = role === 'ai';
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '40px minmax(0,1fr)', gap: 16, alignItems: 'start', ...style }}>
      ...
      {pending ? <span style={{ display: 'inline-flex', gap: 5, alignItems: 'center', height: 26 }}><style>{`@keyframes vc-dot{0%,80%,100%{opacity:.25}40%{opacity:1}}`}</style>{[0, 1, 2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--brand)', animation: `vc-dot 1.2s ${i * .15}s var(--ease-in-out) infinite` }} />)}</span>
