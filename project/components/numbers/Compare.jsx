...
export function Compare({ ours, theirs, ourLabel = 'AI-native production', theirLabel = 'Классическая production-модель', unit = ' ₽', savingLabel = 'разница', note, style }) {
  ...
  const bar = (v, color) => <div style={{ height: 40, background: color, width: on ? (v / max * 100) + '%' : '0%', transition: 'width var(--dur-reveal) var(--ease)', borderRadius: 2 }} />;
  const row = (label, v, vN, color) => (
