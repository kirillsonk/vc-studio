...
export function DemoBlock({ title, meta, poster, ratio = '16/9', cta = 'Запустить демо', autoload = false, loadingText = 'Загружаем сцену', dark = false, children, onStart, style }) {
  ...
  const bg = dark ? 'var(--contrast)' : 'var(--surface)', fg = dark ? 'var(--contrast-ink)' : 'var(--text)', fg2 = dark ? 'var(--contrast-ink-2)' : 'var(--text-2)', ln = dark ? 'var(--contrast-line)' : 'var(--line)';
  return (
      ...
      <div style={{ position: 'relative', aspectRatio: ratio, borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: `1px solid ${state === 'live' ? 'var(--brand)' : ln}`, background: bg, transition: 'border-color var(--dur-ui)' }}>
        {state === 'live' ? children : (
          <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center' }}>
            {poster}
            {state === 'idle' ? (
              <button type="button" onClick={start} style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 10, height: 'var(--control-h-lg)', padding: '0 24px 0 18px', borderRadius: 'var(--radius-md)', border: 'none', cursor: 'pointer', background: 'var(--brand)', color: '#fff', font: '500 17px/1 var(--font-sans)' }}><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>{cta}</button>
            ) : (
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12, font: 'var(--type-body-sm)', color: fg2 }}><span style={{ width: 14, height: 14, borderRadius: '50%', border: '1.5px solid var(--brand)', borderRightColor: 'transparent', animation: 'vc-spin .8s linear infinite' }} /><style>{`@keyframes vc-spin{to{transform:rotate(360deg)}}`}</style>{loadingText}</div>
            )}
          </div>
        )}
      </div>
