...
export function OptionBlock({ selected, disabled, onClick, children, meta, style }) {
  const [hover, setHover] = React.useState(false);
  return (
    <button type="button" disabled={disabled} aria-pressed={!!selected} onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, minHeight: 52, padding: '0 16px', textAlign: 'left', borderRadius: 'var(--radius-md)', font: 'var(--type-body-sm)', fontFamily: 'var(--font-sans)', cursor: disabled ? 'not-allowed' : 'pointer', boxSizing: 'border-box',
