...
export function CaseCard({ title, kind, year, client, days, budget, media, ratio = '4/3', href, onClick, size = 'md', style }) {
  const [hover, setHover] = React.useState(false);
  const Wrap = href ? 'a' : 'div';
  const fact = (v, l) => <span style={{ display: 'flex', flexDirection: 'column', gap: 2 }}><span data-num style={{ font: 'var(--type-body)', fontFeatureSettings: 'var(--num-features)', color: 'var(--text)' }}>{v}</span><span style={{ font: 'var(--type-caption)', color: 'var(--text-3)' }}>{l}</span></span>;
  return (
    <Wrap href={href} onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ display: 'flex', flexDirection: 'column', gap: 20, color: 'var(--text)', textDecoration: 'none', cursor: 'pointer', minWidth: 0, ...style }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {(days || budget) && <div style={{ display: 'flex', justifyContent: 'space-between', gap: 24, paddingTop: 12, borderTop: '1px solid var(--line)' }}>{days && fact(days + ' дней', 'до запуска')}{budget && fact(budget.toLocaleString('ru-RU') + ' ₽', 'бюджет проекта')}</div>}
      </div>
    </Wrap>
  );
}
