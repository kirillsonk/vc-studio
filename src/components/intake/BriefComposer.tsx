...
/** Free-text answer field for the brief: white, 1px, brand send. Enter sends, Shift+Enter breaks. */
export function BriefComposer({ placeholder = 'Или напишите своими словами', value, onChange, onSend, disabled, style }: BriefComposerProps) {
  const [inner, setInner] = React.useState('');
  const [focus, setFocus] = React.useState(false);
  const text = value !== undefined ? value : inner;
  const set = (v: string) => { if (value === undefined) setInner(v); onChange && onChange(v); };
