...
export interface EstimatePanelProps {
  ...
  term?: string;
  budget?: string;
  status?: 'empty' | 'thinking' | 'building' | 'ready';
  onConfirm?: () => void;
  confirmLabel?: string;
...
export function EstimatePanel({ scope = [], term, budget, status = 'empty', onConfirm, confirmLabel = 'Получить точную оценку', style }: EstimatePanelProps) {
  const row: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', gap: 16, padding: '14px 0', borderBottom: '1px solid var(--line)', alignItems: 'baseline' };
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--radius-md)', padding: 24, display: 'flex', flexDirection: 'column', minWidth: 0, ...style }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingBottom: 12, borderBottom: '1px solid var(--text)' }}>
        <span style={{ font: 'var(--type-index)', color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Предварительный scope</span>
        {status === 'thinking' && <span style={{ font: 'var(--type-caption)', color: 'var(--brand)' }}>Собираем scope</span>}
        {status === 'ready' && <span style={{ font: 'var(--type-caption)', color: 'var(--positive)' }}>Готово</span>}
      </div>
