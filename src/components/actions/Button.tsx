...
export interface ButtonProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'style'> {
  /** primary = vermilion fill; secondary = 1px border; tertiary = text with animated baseline */
...
type SizeSpec = [height: string, padX: number, font: string];
...
/** Primary (vermilion) / secondary (1px border) / tertiary (text with animated baseline). Labels are verbs, no trailing arrows. */
export function Button({ variant = 'primary', size = 'md', disabled, loading, icon, iconRight, full, children, style, ...rest }: ButtonProps) {
